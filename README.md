...
| 🚫 **키워드 즉시 차단** | 성인·도박 키워드(한/영 30종+)를 페이지 타이틀에서 감지 → 경고 페이지 즉시 전환 |
| 🧠 **온디바이스 AI** | ONNX WASM — 외부 서버 통신 없이 브라우저 내 실시간 추론 |
| 🔞 **성인 이미지 자동 블러** | 온디바이스 NSFW 분류 모델 — 이미지 데이터 외부 전송 없음 |
| 🔗 **우클릭 링크 탐지** | 직접 방문 없이 링크 URL만으로 사전 분석 후 알림 발송 |
| 🔔 **자동 알림 · 차단** | 임계값 초과 시 Chrome 알림 + 유해 사이트 차단 페이지 자동 리디렉션 |
| 🎨 **동적 아이콘** | 탐지 결과에 따라 툴바 아이콘 색상 실시간 변경 (초록/파랑/주황/빨강) |
| 📊 **탐지 히스토리** | 일간 / 주간 통계 도넛 차트 + Full URL 로그 + 판정별 카테고리 필터링 (최대 999건) |
| ⚙️ **맞춤 설정** | 알림·차단 임계값 슬라이더 개별 조정, 다크모드, 성인·도박 차단, 히스토리 초기화 |
