...
방문하는 모든 URL을 실시간으로 분석하여 **서버 DB 조회(화이트/블랙리스트) → 온디바이스 ONNX 추론**의 2단계로 탐지합니다.
...
결과 처리:
  is_exist = false  → 2단계 (로컬 ONNX 추론) 으로 fallback
...
서버 API 호출 시 매 요청마다 서명을 생성하여 위변조·재전송 공격을 방지합니다.
...
전송 헤더:
  X-EXT-TIMESTAMP  : Unix timestamp (초 단위)
  X-EXT-NONCE      : 무작위 UUID v4
  X-EXT-SIGNATURE  : hex 인코딩된 HMAC-SHA256 서명
...
├── model/
│   └── CatBoost_merged.onnx  # 변환된 추론 모델 (~2.9 MB)
├── lib/
